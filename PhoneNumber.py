...
class PhoneNumber:
    ...
    def __init__(self, number='0000000000'):
        ...
        if PhoneNumber.is_valid(number_without_punctuation):
            ...
            raise InvalidPhoneNumber(f"The Phone Number you have provided,"
                                     f" {number_without_punctuation} "
                                     f"is not a valid US Number")
    ...
    def set_area_code(self, area_code):
        if len(area_code) == 3:
            ...
            raise InvalidPhoneNumber(f"The area code is not the correct \
                                     length to give a valid phone number.")
    ...
    def set_exchange(self, exchange_number):
        if len(exchange_number) == 3:
            self._number = (self._number[0:3] + exchange_number +
                            self._number[6:])
        else:
            raise InvalidPhoneNumber(f"The exchange number is not the correct "
    ...
    def set_subscriber_number(self, subscriber_number):
        if len(subscriber_number) == 4:
            ...
            raise InvalidPhoneNumber(f"The subscriber number is not the "
                                     f" correct length to get "
    ...
    def strip_punctuation(number):
        ...
        return number.translate(str.maketrans("", "", chars_to_remove))
    ...
    def is_valid(number_to_check):
        ...
        if not isinstance(number_to_check, str) or \
        ...
        these will check if there are certain reserved prefixes in the number
        that are not part of v valid number
        '''
        if re.match(r'^\d{10}$', number_to_check):
            if (re.match(r'^[2-9]9', number_to_check) or
                    re.match(r'^37|96', number_to_check) or
                    re.match(r'^[2-9]11', number_to_check)):
                return False
            else:
                return True
